...
#Rasp IP -- Probe module has to receive data from 5006 udp port
UDP_IP='127.0.0.1'
UDP_PORT=5006
...
class ARProbe(mp_module.MPModule):
    print('AeroRiver Probe LogSaver')
    def __init__(self, mpstate):
        super(ARProbe, self).__init__(mpstate, "ar", "logsaver")
        self.data_process = None
        self.collecting_data = False
        self.add_command('ar3', self.cmd_stop, 'AERORIVERPROBE')
        ...
        try:
            while self.collecting_data:
                ...
                try:
                    ...
                    pt, bat1, bat2 = float(val[0]), float(val[1]), float(val[2])
                except socket.timeout:
                    pt, bat1, bat2 = -1, -1, -1
                #-------------------#
                self.master.mav.probe_aeroriver_send(pt, bat1, bat2)
        except KeyboardInterrupt:
            print('Exiting')
    
def init(mpstate):
    '''initialize module'''
    return ARProbe(mpstate)